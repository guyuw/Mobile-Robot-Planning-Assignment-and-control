...
inline void Astarpath::AstarGetSucc(
    MappingNodePtr currentPtr, vector<MappingNodePtr>& neighborPtrSets,
    vector<double>& edgeCostSets) {
  neighborPtrSets.clear();
  edgeCostSets.clear();
  ...
  // 遍历加速度组合 (-max_acc ~ max_acc)
  for (double ax = -max_acc; ax <= max_acc; ax += max_acc) {
    for (double ay = -max_acc; ay <= max_acc; ay += max_acc) {
      for (double az = -max_acc; az <= max_acc; az += max_acc) {
        // 计算新速度
        Vector3d new_velocity = currentPtr->velocity + delta_t * Vector3d(ax, ay, az);

        // 计算新位置
        Vector3d new_position =
            currentPtr->coord + currentPtr->velocity * delta_t +
            0.5 * Vector3d(ax, ay, az) * delta_t * delta_t;

        // // 检查是否越界或碰撞
        Vector3i new_index = coord2gridIndex(new_position);
        // if (isOccupied(new_index))
        //   continue;

        // 创建邻居节点
        MappingNodePtr neighbor = Map_Node[new_index(0)][new_index(1)][new_index(2)];
        neighbor->coord = new_position;
        neighbor->velocity = new_velocity;
        neighbor->time = currentPtr->time + delta_t;

        // 计算代价
        double edge_cost = delta_t; // 时间代价为步长
...
double Astarpath::getHeu(MappingNodePtr node1, MappingNodePtr node2) {
  Vector3d pos_diff = node2->coord - node1->coord;
  double dist = pos_diff.norm(); // 欧几里得距离
  double vel_magnitude = max(node1->velocity.norm(), 1e-3); // 避免速度为零

  // 时间估计：距离 / 速度
  double time_cost = dist / vel_magnitude;

  // Tie breaker 加速收敛
  double tie_breaker = 1.0 + 1.0 / 10000.0;

  return time_cost * tie_breaker;
}




// bool Astarpath::AstarSearch(Vector3d start_pt, Vector3d end_pt, Vector3d start_vel) {
//   ros::Time time_1 = ros::Time::now();

//   // start_point 和 end_point 索修改邻居扩展函数 (AstarGetSucc)引
...
bool Astarpath::AstarSearch(Vector3d start_pt, Vector3d end_pt, Vector3d start_vel) {
  ...
  while (!Openset.empty()) {
    // 从 OpenSet 中取出 f(n) 最小的节点
    currentPtr = Openset.begin()->second;
    Openset.erase(Openset.begin());

    // 判断是否到达目标区域
    if ((currentPtr->coord - endPtr->coord).norm() < resolution &&
        currentPtr->velocity.norm() < 1e-2) {
      terminatePtr = currentPtr;
      ROS_INFO("Kino-Dynamic A* Search Completed.");
      return true;
    }
    ...
    for (size_t i = 0; i < neighborPtrSets.size(); ++i) {
      ...
      if (neighborPtr->id == 0) { // 新节点
        neighborPtr->g_score = tentative_g_score;
        neighborPtr->f_score = neighborPtr->g_score + getHeu(neighborPtr, endPtr);
        neighborPtr->Father = currentPtr;
        neighborPtr->id = 1;
        Openset.insert(make_pair(neighborPtr->f_score, neighborPtr));
      } else if (neighborPtr->id == 1) { // OpenSet 中已存在
  ...
  ROS_WARN("Kino-Dynamic A* Search Failed.");
  return false;
}
